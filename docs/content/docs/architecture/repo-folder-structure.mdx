...
Phase 1.1 adds the static typed provider definition catalog in `self/subcortex/providers/src/definitions/`. The catalog contains metadata-only definitions for Anthropic, Chat Completions-compatible remote providers, and Ollama, and exports literal-derived provider key types. Runtime bootstrap and credential loading still use the current provider-registration paths until WR-177 Phase 1.3 consumes the catalog.
