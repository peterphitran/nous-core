...
└── runtime/                           # ProviderRegistry, CLI session manager, and inference-runtime composition
```
...
The static typed provider definition catalog is generated directly into `self/subcortex/providers/src/provider-definitions.ts`; each provider metadata constant lives with its owning provider leaf. The catalog contains metadata-only definitions for Anthropic, OpenAI Chat Completions, and Ollama, and exports literal-derived provider key types. Concrete provider-owned adapter modules live in provider leaves such as `self/subcortex/providers/src/providers/anthropic/adapter.ts` and `self/subcortex/providers/src/providers/ollama/adapter.ts`. Shared protocol adapters live under `self/subcortex/providers/src/protocols/`; the OpenAI leaf wraps `self/subcortex/providers/src/protocols/openai-api/`.
...
Agent-CLI provider lifecycle is composed in `self/subcortex/providers/src/runtime/cli-session-manager.ts`. Chat-bound CLI fixtures receive a session-keyed provider wrapper; transient and batch invocations continue to use the provider's one-shot execution path. The session manager derives keys from provider id, fixture role, chat session id, and project id only for project-scoped chat. It tracks process/session lifecycle state, not chat memory.
