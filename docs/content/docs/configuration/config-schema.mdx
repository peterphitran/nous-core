...
- **vendor** (optional): Certified provider vendor key for adapter selection. Known vendor values include `anthropic`, `openai`, `ollama`, and `codex-cli`; see [Provider Adapters](/docs/development/provider-adapters) for how vendor keys map to provider leaves. Unknown values fall back through the adapter resolver rather than becoming certified vendors.
...
For transient and batch work, Codex CLI still runs through the one-shot `codex exec` command path. For chat-bound fixtures, Nous routes the provider through the runtime CLI session manager. The Codex adapter currently declares `session_bound_command`, which means chat turns are mediated by a session-keyed wrapper rather than a strict long-lived process protocol.

The runtime session key is derived from the provider id, chat fixture role, and chat session id. Project-scoped chat also includes `projectId`; system-wide chat omits it. Conversation history remains in Cortex STM, not in provider session state.
...
| `NOUS_CODEX_CLI_BIN` | Overrides the Codex CLI executable used by the certified `codex-cli` provider |
| `CODEX_CLI_BIN` | Fallback Codex CLI executable override when `NOUS_CODEX_CLI_BIN` is not set |
